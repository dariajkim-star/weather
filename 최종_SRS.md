...
| 시나리오 A | 기존 집중호우 시나리오 (부산/오송/강남 실제 사례) |
| 시나리오 B | 태풍 근접 통과 — 비 강함·바람 강함·하천범람 가능·침수 속도 중간 |
| 시나리오 C | 태풍 직접 통과 — 비 매우 강함·바람 매우 강함·하천범람 발생·침수 속도 빠름 |
...
| FR-22 | 시나리오 B(태풍 근접)와 C(태풍 직접)를 추가로 지원해야 한다. 각 시나리오는 태풍 거리·이동 방향·이동 속도·예상 강수량·바람 세기·하천범람 가능성 변수를 가지며, 이를 수위 상승 속도·장비 효율·침수 곡선 유형에 반영해야 한다. | Must | 태풍 시나리오 |
